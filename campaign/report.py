...
def main():
    ...
    corr += '<p>Each assertion compares one observed value with an expected value. Counts include repeated checks across builds and runs; they are not counts of distinct tests or bugs. Harness errors count Python/harness exceptions, not PostgreSQL crashes (which are failed restart checks).</p>'
    corr += '<details><summary>All assertions: expected and actual values</summary>'
    for suite in suites:
        corr += f'<h3>{ESC(suite["name"])}</h3>'
        corr += table(['Assertion', 'Status', 'Actual', 'Expected'], [[ESC(r.get('name', '')), ESC(r.get('status', '')), ESC(json.dumps(r.get('actual'))), ESC(json.dumps(r.get('expected')))] for r in suite['assertions']])
    corr += '</details>'
    findings = '' 
    ...
    # Email drafts are private working material, never a report artifact.
    (args.output/'hackers-draft.txt').unlink(missing_ok=True)
...
<footer class="card"><strong>Reproduce. Challenge. Extend.</strong><p>Negative findings are retained, pending work stays visible, and each reported number traces to a supplied artifact.</p></footer></main></body></html>'''
