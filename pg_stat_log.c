 ...
 *
 * In addition to recording the reset baseline, zero the main stats block
 * (slot array + num_entries) and the reset_offset so that slots are
...
pg_stat_log_reset_all_cb(TimestampTz ts)
{
	...
	MemSet(reset->entries, 0,
		   (Size) reset->max_entries * sizeof(PgStatLogSlot));
	reset->num_entries = 0;
...
pg_stat_log_emit_hook(ErrorData *edata)
{
	...
	{
		/* Dropped silently when full — track for visibility */
		shmem->n_dropped++;
	}
...
pg_stat_log_info(PG_FUNCTION_ARGS)
{
	...
	/* Read the four fields consistently under a shared lock */
	LWLockAcquire(&shmem->lock, LW_SHARED);
